...
def train_iteration_multi_gpu(model, optimizer, data_loader, epoch, args):
    ...
    for record in data_loader:
        ...
        with autocast():
            retriever_ce_loss, generation_loss = model(**record)
        loss = args.alpha*retriever_ce_loss + args.beta*generation_loss
        torch.distributed.barrier() 
        reduced_loss = reduce_tensor(loss.data)
        total_loss += reduced_loss.item()
        try:
            total_generation_loss += float(args.beta*generation_loss.cpu().detach().numpy())
        except:
            total_generation_loss += 0
        ...
        if step%args.report==0 and local_rank==0:
            ...
            h, m = divmod(m, 60)
            local_start = time.time()
            print(f"epoch:{epoch} training step: {step}/{all_steps_per_epoch}, mean loss: {total_loss/step}, ce loss: {total_ce_loss/step}, gen loss: {total_generation_loss/step}, ", "report used time:%02d:%02d:%02d," % (h, m, s), end=' ')
